...
PartFile::PartFile(const std::string &file_name) {
...
Chunk* PartFile::read_chunk(int64_t offset, int64_t len) {
...
void PartFile::write_chunk_next(Chunk* chunk) {
	...
	for (int64_t i = 0; i < len; i += WRITE_IO_SIZE) {
		...
		int ret = pwrite(fd, in_buffer + i, io_size, file_size);
		assert(ret == io_size);
		file_size += io_size;
	}
}
