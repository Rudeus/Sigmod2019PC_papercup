...
//			assert(ret == 0);
		...
		int64_t prefetch_pid = part_index + task_manager->part_num / 3;
		if (prefetch_pid >= task_manager->part_num) continue;
		int64_t prefetch_offset = task_manager->part_offset[prefetch_pid];
		int64_t prefetch_len = task_manager->partition_lens[prefetch_pid];
		for (int64_t i = 0; i < prefetch_len; i+= 512) {
			//memcpy(write_output->file_map + prefetch_offset + i, prefetch_buf, 1);
			memcpy(prefetch_buf, write_output->file_map + prefetch_offset + i, 1);
		}
	}
}
...
//	print_time("Prefetch thread off");
	...
	if (counter == task_manager->prefetch_len) {
